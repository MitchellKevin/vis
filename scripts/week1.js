...
chapterInit['ch-ring'] = (el) => {
  ...
  // Dag-scheidingslijnen, labels en nachtbanden — één loop
  for (let d = 0; d < DAYS; d++) {
    ...
    svg.append('text')
      .attr('x', cx + Math.cos(aLabel) * (outerR + 24))
      .attr('y', cy + Math.sin(aLabel) * (outerR + 24) + 4)
      .attr('text-anchor', 'middle')
      .attr('font-family', 'DM Mono, monospace').attr('font-size', 13).attr('letter-spacing', '0.12em')
      .attr('fill', d >= 1 && d <= 7 ? '#6941c6' : '#01463c').attr('opacity', 0.85)
  ...
  pings.forEach(p => {
    ...
    const showDetail = () => {
      ...
      detailPanel.innerHTML = `
        ...
        Piek: ${MONTH_FULL[p.monthly.indexOf(Math.max(...p.monthly))]}<br/>
        Diepte: ${p.diepte}<br/>
        Gewicht: ~${p.weight} kg
      `;

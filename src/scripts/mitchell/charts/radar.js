...
import { COLORS, FONT_BODY, MONTH_FULL } from '../constants.js';
...
export function initRadar() {
  ...
  const minCount = positiveCounts.length ? Math.min(...positiveCounts) : maxCount;
  const innerDist = 70, outerDist = R - 55;
  const distanceScale = d3.scaleSqrt().domain([minCount, maxCount]).range([outerDist, innerDist]).clamp(true);
  const minGap = 70; // minimale onderlinge afstand in px
  const pings = [];
  visData.forEach(v => {
    const distance = v.count > 0
      ? distanceScale(v.count) + (pingSeed() - 0.5) * 22
      : outerDist + 10;
  ...
  pings.forEach(p => {
    ...
    const labelText = `${p.naam} · ${formatNumber(p.count)}`;
    const estWidth = labelText.length * 7 + 26;
    if (labelRight && p.x + estWidth > W - 6) labelRight = false;
    else if (!labelRight && p.x - estWidth < 6) labelRight = true;
    g.append('text')
    ...
    const showDetail = () => {
      ...
      detailPanel.innerHTML = `<strong>${p.naam}</strong>${formatNumber(p.count)} waarnemingen<br/>Piek: ${MONTH_FULL[p.monthly.indexOf(Math.max(...p.monthly))]}<br/>Diepte: ${p.diepte}<br/>Gewicht: ~${p.weight} kg`;
  ...
  if (summaryEl && topSpecies) {
    summaryEl.textContent = `De radar detecteert ${visData.length} vissoorten. Meest gesignaleerd: ${topSpecies.naam} (${formatNumber(topSpecies.count)} waarnemingen).`;
  }

  let angle = -Math.PI / 2, frameId = 0, running = false;
  const revealed = new Set();
  function tick() {
    angle += reduceMotion() ? 0.03 : 0.01;
    sweep.attr('transform', `rotate(${angle * 180 / Math.PI} ${cx} ${cy})`);
    pings.forEach((p, i) => {
      const angleDelta = (Math.atan2(p.y - cy, p.x - cx) - angle + Math.PI * 4) % (Math.PI * 2);
      if (!revealed.has(i) && angleDelta < 0.18) { revealed.add(i); p.elem.transition().duration(300).attr('opacity', 1); }
    });

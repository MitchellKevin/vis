...
export function renderChoropleth(ctx) {
  ...
  const scale = d3.scaleSequentialLog(d3.interpolate('#C8E6D0', '#1B4332')).domain([1, mx]);
  countriesG.selectAll('path').attr('fill', d => {
    const c = cdata[d.id]; return c ? scale(c.events) : C.land;
  });
  overlaysG.selectAll('*').remove();
  return { type: 'gradient', title: 'Bezoeken', from: '#C8E6D0', to: '#1B4332', lo: 'Weinig', hi: 'Veel' };
}
...
export function renderUploadRate(ctx) {
  ...
  overlaysG.selectAll('*').remove();
  return { type: 'gradient', title: 'Upload ratio', from: '#d73027', to: '#1a9850', lo: 'Gesloten', hi: 'Gespot' };
}
...
export function renderDevice(ctx) {
  ...
  const MOBILE_COLOR  = '#7B4FBF';
  const DESKTOP_COLOR = '#F5A623';
...
export function renderTime(ctx) {
  ...
  return {
    type: 'rows', title: 'Actief tijdstip', rows: [
      { color: '#5C35A8', label: 'Nacht (0–6u)'    },
      { color: '#F5C842', label: 'Ochtend (6–12u)' },
      { color: '#E8896A', label: 'Middag (12–18u)' },
      { color: '#3A7D44', label: 'Avond (18–24u)'  },
    ],
  };
}

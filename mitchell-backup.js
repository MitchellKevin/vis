 ...
 * ─────────────────────────────────────────────────────────────
 * Een scrollytelling datavisualisatie in één HTML-pagina.
 * Dit script verzorgt alle JavaScript voor de 14 hoofdstukken.
 *
 * Architectuur
 * ────────────
 * 1. DATA            — statische vissoort-data + afgeleide tijdreeksen
 * 2. PRNG            — reproduceerbare willekeur (vaste seed zodat data
 *                      bij elke reload identiek is)
 * 3. DATA GENERATORS — maand- en dagverdeling op basis van Gaussische piek
 * 4. UTILITIES       — DOM-helpers, nummeropmaak, prefers-reduced-motion
 * 5. SVG FISH        — herbruikbare SVG-vis-render helpers
 * 6. TOOLTIP         — gedeelde zwevendige tooltip (alle hoofdstukken)
 * 7. CHAPTER NAV     — vaste zijbalk met hoofdstuk-dots
 * 8. OBSERVER        — IntersectionObserver die elk hoofdstuk lazy-init
 *                      zodra het in beeld komt
 * 9. CHAPTERS 0–12   — één init-functie per hoofdstuk, gekoppeld aan het
 *                      chapterInit-object; canvas-visualisaties pauzeren
 *                      automatisch als ze buiten beeld zijn
 * 10. BOOTSTRAP      — observers registreren, nav bouwen, bubbles starten
 *
 * Externe afhankelijkheden
 * ────────────────────────
 * D3 v7       — sankey, pack, line, schalen (CDN)
 * d3-sankey   — gebruikt in hoofdstuk 3 (via CDN)
 * Google Fonts — Fraunces + DM Mono (in <head> van index.html)
 *
 * Toegankelijkheid
 * ────────────────
 * - prefers-reduced-motion: animaties uitgeschakeld, één statisch frame
 * - Alle interactieve elementen: tabindex, aria-label, toetsenbord-events
 * - Live regions (aria-live) op tellers en detail-panelen
 */
  ...
  // Mulberry32 — snel en reproduceerbaar. Vaste seed zodat visualisaties
  // identiek laden bij elke page refresh (geen surprise-layouts).
  ...
  // MARK: generateMonthly
  // Verdeelt het totaal over 9 maanden (maart–november) via een
  // vaste piekverhouding. April en mei zijn het grootst (paaigedrag).
  ...
  // MARK: generateDaily
  // Produceert 365 dagwaarden via een Gauss-curve rond dag 110 (~20 april).
  // Buiten het seizoen (voor dag 60 / na dag 305) is de waarde 0.
  ...
  // MARK: - SVG Fish Helpers

  // MARK: fishSymbolId
  // Geeft het juiste SVG-symbol-ID terug op basis van vistype.
  // vintage=true geeft de etching-variant (voor het journaal).
  function fishSymbolId(shape, vintage = false) {
    return (vintage ? 'fish-etch-' : 'fish-') + shape;
  }
  // MARK: fishUse
  // Geeft een SVG <g>-string terug met de vis op positie (x,y),
  // breedte w en optionele rotatie, animatie en kleur.
  ...
  // MARK: showTooltip
  // Toont de gedeelde tooltip op schermpositie (x, y) met HTML-inhoud.
  // Meerdere aanroepen binnen dezelfde frame annuleren elkaars timer.
  ...
  // MARK: hideTooltip
  // Verbergt de tooltip, optioneel met een vertraging in ms.
  ...
  // MARK: buildChapterNav
  // Bouwt de vaste zijbalk (rechts) met één dot per hoofdstuk.
  // Actieve dot volgt de scroll via een tweede IntersectionObserver
  // (rootMargin -45% zodat het middelpunt van de viewport telt).
  // De nav is onzichtbaar zolang de dive-sectie in beeld is.
  ...
  // MARK: - Section Observer

  // chapterInit is een object waarbij elke sleutel het id van een
  // <section> is. De bijbehorende functie wordt precies één keer
  // aangeroepen zodra de sectie de viewport binnengaat.
  ...
  // MARK: spawnBackgroundBubbles
  // Voegt periodiek kleine <div class="bubble"> elementen toe aan .atmosphere.
  // Animatie via CSS custom properties (--drift, --max-op).
  // Wordt overgeslagen als prefers-reduced-motion actief is.
  ...
  // MARK: - Chapter 0: De Duik

  // Sticky scroll-effect: de sectie is 220vh hoog zodat scrollen
  // de overgang van boven water (ochtendlicht + Domtoren) naar
  // onderwater stuurt. t loopt van 0→1 als percentage van de scroll.
  chapterInit['ch-dive'] = (el) => {
    ...
    // MARK: update (duik-scroll)
    // Berekent t ∈ [0,1] op basis van scrollpositie binnen de sticky sectie
    // en past opacity/transform van de visuele lagen proportioneel aan.
    function update() {
  ...
  // MARK: - Chapter 1: De Visdeurbel-Knop

  // Vissen cirkelen in concentrische banen via rAF (requestAnimationFrame).
  // Elke soort krijgt een eigen baan (radius prop. aan rang).
  // Press-and-hold: na 420ms start een interval van 260ms per belroep.
  // De boost-variabele versnelt alle vissen tijdelijk bij elke klik.
...
//     // MARK: tick (bel-animatie)
//     // rAF-loop: berekent positie van elke vis op zijn cirkelbaan.
//     // boost vervaagt exponentieel (× 0.94 per frame) na een klik.
//     function tick(now) {
//       const dt = now - lastT;
//       lastT = now;
...
//     // MARK: press
//     // Verwerkt één beldruk: teller omhoog, boost toevoegen, ripple spawnen.
//     function press() {
//       clicks++;
...
//     let holdInterval = 0;
//     let isHolding = false;
//     // MARK: startHold / endHold
//     // startHold roept direct press() aan en start na 420ms een interval
//     // voor automatisch herhalen. endHold wist beide timers en verwijdert
//     // de .holding CSS-klasse die de pulseer-animatie aanzet.
//     function startHold() {
//       if (isHolding) return;
//       isHolding = true;
//       press();
...
//   };
  ...
  // MARK: - Chapter 2: Zwemrace

  // Elke baan is een CSS-grid rij (naam | balk | aantal).
  // Breedte van .race-progress wordt ingesteld via CSS-transitie op scroll-in.
  // Klik of Enter herstart de animatie: reset naar 0%, force reflow,
  // dan opnieuw breed animeren + tijdelijk .dart class.
...
//   // MARK: - Chapter 3: Sluis-Doorgang

//   // Verticale SVG-doorsnede van de Weerdsluis, opgebouwd uit:
//   // - Stenen zijwanden met willekeurige textuurrechthoeken
//   // - Kubische Bézier-paden per soort (dikte = aantal, kleur = soort)
//   // - Hover op een stroom: .has-focus op de container dimt alle andere
//   //   stromen via CSS (.sluis-streams.has-focus .sluis-stream { opacity })
...
//   // MARK: - Chapter 4: Seizoensklok

//   // Radiale grafiek met D3. Elke van de top-5 soorten krijgt een ring;
//   // per maand zet de ring uit/in op basis van het maandaandeel.
//   // Interactie: hover ring → highlight via .has-focus/.active CSS-klassen.
//   // Hover maand-wedge → onzichtbaar klikgebied over de taartstuk,
//   // toont alle 5 soortwaarden voor die maand in de tooltip.
...
//   // MARK: - Chapter 5: Diepte-Strata

//   // Drie volle-breedte lagen (top/mid/bodem) elk met een eigen SVG.
//   // Vissen worden geplaatst aan de hand van hun `diepte`-veld in visData.
//   // Aantallen bepalen hoe veel vissen per soort verschijnen (max 12).
...
//   };
  ...
  // MARK: - Chapter 6: Aquarium

  // Canvas-gebaseerd. ~80 vis-deeltjes op basis van proportionele steekproef.
  // Elke vis heeft vx/vy + zachte trek naar het midden + willekeurige ruis.
  // Sprites: per (shape, color) wordt één offscreen <canvas> gecacht voor
  // goedkope drawImage-calls in de render-loop.
  // Klik → krachtsimpuls vanuit klikpunt (scatter).
  // Hover → dichtstbijzijnde vis binnen 4% viewport-eenheden = tooltip.
  // Filter-chips → f.visible vlag aan/uit; onzichtbare vissen worden
  // niet getekend maar bewegen wel door (anders hergroeperen ze na toggle).
  chapterInit['ch-aquarium'] = (el) => {
    ...
    // MARK: resize (aquarium)
    // Past canvas-resolutie aan op devicePixelRatio voor scherp beeld
    // op retina-schermen. Aangeroepen bij init en op window resize.
    function resize() {
    ...
    const sprites = {};
    // MARK: makeSprite
    // Rendert een vis eenmalig naar een offscreen canvas (gecacht op shape+color)
    // zodat drawImage in de rAF-loop goedkoop blijft.
    ...
    // MARK: drawFishPath
    // Tekent een viseenvorm direct op een Canvas 2D-context.
    // Vijf varianten: round, baars, pred (roofvis), long (paling) en tiny (alver).
    ...
    // MARK: tick (aquarium)
    // Beweegt alle vis-deeltjes, tekent sprites via drawImage.
    // Sla over als f.visible=false (gefilterd via filter-chips).
    function tick() {
    ...
    // MARK: animateCounter
    // Telt de teller op van 0 naar TOTAL in ~3.8 seconden via
    // een ease-in-out curve. Wordt één keer gestart bij de eerste scroll-in.
    let counterRaf = 0;
  ...
  // MARK: - Chapter 7: Stroming

  // Canvas-rivier. Lanen zijn proportioneel aan visaantallen (yStart/yEnd).
  // Elk deeltje volgt een sinusgolf-pad: y = baseY + sin(x * freq + phase) * amp.
  // De x-component loopt lineair en wraps bij 110%.
  // Achtergrondstrepen geven extra gevoel van waterbeweging.
//   chapterInit['ch-flow'] = (el) => {
//     const canvas = $('#flowCanvas');
//     const ctx = canvas.getContext('2d');
//     let W = 0, H = 0, dpr = window.devicePixelRatio || 1;
//     // MARK: resize (stroming)
//     function resize() {
//       const r = canvas.parentElement.getBoundingClientRect();
//       W = r.width; H = r.height;
//       canvas.width = W * dpr; canvas.height = H * dpr;
...
//     // MARK: tick (stroming)
//     // Tekent sinusgolf-paden per deeltje en horizontale stromingsstrepen.
//     // t telt frames en geeft de golven een tijdsdimensie.
//     function tick() {
//       ctx.clearRect(0, 0, W, H);
...
//     // MARK: drawFishSimple
//     // Vereenvoudigde vis-vorm voor de stromings-canvas (sneller dan makeSprite).
...
//   };
  ...
  // MARK: - Chapter 8: Ringkalender

  // SVG-cirkel met 365 stipjes via D3. Radius en opacity van elk stipje
  // zijn proportioneel aan dailyTotals[d]. Stippen buiten het seizoen
  // (jan–feb, nov–dec) zijn 0 en niet hoverbaar.
  // Hover → dayToDateString() zet dagnummer om naar leesbare datum.
  chapterInit['ch-ring'] = (el) => {
    ...
    // MARK: dayToDateString
    // Zet een dagnummer (0=1 jan) om naar een leesbare Nederlandse datum.
  ...
  // MARK: - Chapter 9: Visradar

  // Sonar-animatie via SVG rotate-transform op een sweep-pad.
  // Elke ping heeft een hoek bepaald door een vaste PRNG-seed;
  // zodra de sweep de hoek passeert (da < 0.18 rad) wordt de ping
  // zichtbaar gemaakt via opacity-transitie.
  // Klik op ping → detailpaneel toont soort, piekmaand, diepte, gewicht.
  chapterInit['ch-radar'] = (el) => {
    ...
    pings.forEach((p, i) => {
      ...
      // MARK: showDetail (radar)
      // Deselecteert alle andere pings, selecteert deze en vult het
      // detail-paneel linksonder met soortinfo.
      const showDetail = () => {
  ...
  // MARK: - Chapter 10: Onderwaterlandschap

  // Brede SVG met 5 habitat-zones (riet, zand, steen, open, oppervlak).
  // Elke zone is een <g class="habitat-zone"> met:
  //   - Een decoratieve laag (stengels, stenen, zandvlek…)
  //   - Vissen uit visData gefilterd op v.habitat
  // Hover/focus per zone → floating tooltip via de gedeelde tooltipEl.
...
//   };
  ...
  // MARK: - Chapter 11: Hengel-Vangst

  // D3 circle-packing met drie wisselbare statistieken:
  //   count   → aantal waarnemingen
  //   weight  → gemiddeld gewicht per vis
  //   biomass → count × weight (totale biomassa)
  // renderPack() is herbruikbaar: enter/update/exit-patroon voor
  // soepele transitie bij toggle. Gradients worden eenmalig aangemaakt
  // en gehergebruikt via gradId (gebaseerd op soortnaam).
  chapterInit['ch-net'] = (el) => {
    ...
    // MARK: renderPack
    // (Her)bouwt de bubbel-pack op basis van de geselecteerde statistiek.
    // Gebruikt D3 enter/update/exit voor soepele transitie bij toggle.
    // Gradients worden eenmalig aangemaakt via gradId (soortnaam als sleutel).
  ...
  // MARK: - Chapter 12: Journaal

  // Krant-stijl met uitklapbare entries. Elke <article> heeft een
  // verborgen .journal-extra div (max-height: 0 → max-height: 320px
  // bij .expanded). De `stories`-teksten geven ecologische context.
  ...
  // Registreer alle hoofdstukken bij de sectionObserver zodat elke
  // chapterInit precies één keer wordt aangeroepen bij scroll-in-view.
  $$('.chapter').forEach(c => sectionObserver.observe(c));

  // Vul initiële teksten in die TOTAL nodig hebben (kunnen pas na
  // de data-sectie worden ingesteld).
  ...
  // Bubbels starten na de eerste paint zodat ze de initiële render
  // niet vertragen.
